...
from extract.rating_from_grid import rating_rows_from_section_tables
from extract.section_catalog import (
    SECTION_FINANCIAL,
    SECTION_PRIMARY,
    SECTION_VALID,
)
from extract.evaluation_date import extract_evaluation_date
from extract.sections import extract_section_tables_from_document
...
def extract_credit_report(
    ...
    *,
    ...
    metric_classifier: MetricClassifier | None = None,
) -> dict[str, Any]:
    """단일 PDF를 처리해 결과 객체를 반환한다."""
    settings = get_settings()
    pdf_path = Path(pdf_path)
    active = classifier or LabelClassifier.from_yaml()
    metrics = metric_classifier or MetricClassifier.from_yaml()
    ...
    try:
        ...
        evaluation_date = extract_evaluation_date(first_page_text, pdf_path.name)

        extracted_rows: list[ExtractedRatingRow] = []
        section_grids = extract_section_tables_from_document(
            document, max_pages=pages_to_check
        )

        for key in (SECTION_PRIMARY, SECTION_VALID):
            for grid in section_grids.get(key) or []:
                extracted_rows.extend(
                    rating_rows_from_section_tables({key: grid})
                )
        ...
        fin_tables = [
            grid.to_fin_table()
            for grid in (section_grids.get(SECTION_FINANCIAL) or [])
        ]
        tables_payload = []
        for key in (SECTION_PRIMARY, SECTION_VALID, SECTION_FINANCIAL):
            for grid in section_grids.get(key) or []:
                tables_payload.append(grid.to_dict())

        financial_facts, undefined_metrics = facts_from_fin_tables(
            fin_tables, metrics
        )
        ...
        if rebuild_error:
            ...
            empty["tables"] = tables_payload
            empty["financial_tables"] = [t.to_dict() for t in fin_tables]
            empty["financial_facts"] = [f.to_dict() for f in financial_facts]
            empty["undefined_metrics"] = undefined_metrics
            return empty
        ...
        return {
            ...
            "financial_tables": [t.to_dict() for t in fin_tables],
            "financial_facts": [f.to_dict() for f in financial_facts],
...
def build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="신용평가서 PDF에서 평가대상별 신용등급·재무지표를 추출합니다.",
    )
    parser.add_argument(
        "input",
        type=str,
    ...
    parser.add_argument(
        "--renormalize",
        action="store_true",
        help="문서 DB의 raw 재무지표 그리드만으로 financial_facts를 재생성",
    )
    return parser
