...
def test_primary_and_valid_both_extracted(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "main.rating_rows_from_section_tables",
        lambda tables: [primary, valid],
    )
    monkeypatch.setattr(
        "extract.sections.extract_section_tables",
        lambda page: {},
    )
    monkeypatch.setattr(
...
def test_primary_orphan_table_row_falls_back_to_visual(
    ...
    monkeypatch.setattr(
        "main.rating_rows_from_section_tables",
        lambda tables: [visual],
    )
    monkeypatch.setattr(
        "extract.sections.extract_section_tables",
        lambda page: {"primary_rating": object()},
    )
